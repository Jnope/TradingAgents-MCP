...
class TushareProvider(BaseStockDataProvider):
    ...
    def _get_token_from_database(self) -> Optional[str]:
        """MongoDB token lookup removed; always returns None."""
        return None
    ...
    def connect_sync(self) -> bool:
        ...
        try:
            ...
            if env_token:
                try:
                    self.logger.info(f"🔄 使用环境变量中的 Tushare Token (超时: {test_timeout}秒)...")
                    ...
                    try:
                        ...
                        self.logger.error(f"❌ Tushare Token 测试失败: {e}")
                        return False

                    if test_data is not None and not test_data.empty:
                        self.connected = True
                        self.token_source = 'env'
                        self.logger.info(f"✅ Tushare连接成功 (Token来源: 环境变量)")
                        return True
                    else:
                        self.logger.error("❌ Tushare Token 测试失败")
                        return False
                except Exception as e:
                    self.logger.error(f"❌ Tushare连接失败: {e}")
                    return False

            self.logger.error("❌ Tushare token未配置，请在 .env 文件中配置 TUSHARE_TOKEN")
            return False
            ...
            if env_token:
                try:
                    ...
                    try:
                        ...
                        self.logger.error(f"❌ Tushare Token 测试超时 ({test_timeout}秒)")
                        return False
